...
```json
{
  "settings": {
    ...
    "bg": "#7d9bc1",           // 聊天背景色
    "frame": true,             // 手機外框開關
    "watermark": true,         // 匯出右下角浮水印開關
    "wmText": "LINE 對話製造機", // 浮水印文字,可自訂
    ...
    "sbAlarm": true, "sbArrows": true, "sbVolte": true, "sbSignal": true, "sbBatt": true,  // 狀態列各系統圖示開關(省略=顯示)
    "glow": 0, "glowColor": "#96b9ff",  // 螢幕光暈強度 0-100 與顏色(嵌入展示用)
    "backlight": 0, "backColor": "#06c755",  // 機身背光強度 0-100 與主色(參考站同款橢圓漸層)
    "font": "",                // 整支手機畫面的字體(CSS font-family 字串;空字串=系統預設黑體)
    "skin": "memo",            // 外觀風格(主要):'memo'(預設,貼紙手帳)/'jelly'(果凍軟糖)/'doodle'(手繪塗鴉)/'pop'(波普霓虹暗底);'real'(真實)僅本機 opt-in,對外一律退回 'memo'
    "sysColor": "#2d3b4e",     // 系統顏色:狀態列+表頭底色;僅 skin='real' 時作用(玩樂 skin 自帶配色)。舊欄位 os 已廢棄、忽略
    "theme": "light",          // 'light' 或 'dark':僅 skin='real' 時作用(LINE 深色模式)
    "aiFab": true,             // LINE AI 懸浮鈕(聊天區右下圓鈕)
  ...
  "messages": [
    ...
    { "type": "msg", "side": "right", "text": "自己說的話(綠泡泡)", "time": "下午4:06", "read": "已讀 8",
      "quote": { "name": "被引用者", "text": "被引用的訊息" }, "react": ["😆", "❤️"] },
...
- `react` 選填=訊息下方的表情回應列(灰笑臉+表情,LINE 的「心情」),每項是 emoji 字串或 64px 小圖 dataURL;省略或空陣列=不顯示。
