...
先搞清楚演出管線:你的劇本不會被人類觀眾讀到,而是交給一個「執行 AI」逐字轉譯成一張以假亂真的 LINE 聊天畫面——觀眾最後看到的只有那張截圖。
所以這個舞台沒有旁白、沒有內心獨白、沒有場景與鏡頭描寫、沒有表情動作說明;所有情緒與劇情,都只能用「畫面上真的看得到的東西」演出來:訊息的字句、時間差、已讀、貼圖、引用、表情回應、打了沒送出的草稿。這是 show, don't tell 的 LINE 版——寫任何一行前先問自己:這個東西會出現在截圖上嗎?不會,就換成畫面演得出來的手法。
...
- 文字訊息的台詞行只能寫角色「真的會打出來」的字。轉折/高潮/伏筆這類舞台指示,寫在劇本最後的標註區(第 3 點),不要放在台詞行內——執行端曾把「(轉折)」原樣抄進對話泡泡。
