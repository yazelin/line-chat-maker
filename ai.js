...
async function createImageJob(prompt, size) {
...
async function runFillImages() {
  ...
  try {
    ...
    cells.forEach((c, i) => {
      ...
      const dataUrl = drawSlot(img, col * cw + cw * inset, row * ch + ch * inset, cw * (1 - inset * 2), ch * (1 - inset * 2), c.slot.type);
      if (c.slot.type === 'avatar') { state.people[c.slot.personIndex].avatar = dataUrl; state.people[c.slot.personIndex].avatarPrompt = c.prompt; }
      else { const m = state.messages[c.slot.msgIndex]; m.img = dataUrl; m.imgPrompt = c.prompt; }
    });
    aiUndoStack.push({ draftId: currentId, snap: before });
    if (aiUndoStack.length > 20) aiUndoStack.shift();
