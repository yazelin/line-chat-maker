...
// ── 鏡像:app.js downloadCellImage 的副檔名依實際 data URL 型別走 ──
...
test('下載副檔名:jpeg data URL 的頭像存成 .jpg,不再固定 .png', () => {
  assert.equal(downloadName('data:image/jpeg;base64,AAAA', 'avatar-小白.png'), 'avatar-小白.jpg');
  assert.equal(downloadName('data:image/png;base64,AAAA', 'sticker-3.png'), 'sticker-3.png');
  assert.equal(downloadName('data:image/webp;base64,AAAA', 'avatar-x.png'), 'avatar-x.webp');
  assert.equal(downloadName('not-a-data-url', 'image-2.jpg'), 'image-2.png', '無法辨識型別時退回 png');
});

// ── 鏡像:ai.js validateFillCells 取回時的指紋防呆(草稿變動→跳過對不上的格,不靜默貼錯)──
...
test('取回防呆:草稿未動時全部照原位、無略過', () => {
  const messages = [{ time: '下午3:00', kind: 'sticker', personId: 'p1', side: 'left' }];
  const people = [{ id: 'p1' }, { id: 'p2' }];
  const cells = [
    { type: 'sticker', msgIndex: 0, fp: { time: '下午3:00', kind: 'sticker', personId: 'p1', side: 'left' } },
    { type: 'avatar', personIndex: 1, personId: 'p2' },
  ];
  const v = validateFillCells(cells, messages, people);
  assert.equal(v.skipped, 0);
  assert.equal(v.cells.length, 2, '長度不變才能維持切圖對位');
  assert.ok(!v.cells[0].skip && !v.cells[1].skip);
});

test('取回防呆:淨零筆數編輯(index 指到別則訊息)→ 指紋不符,標 skip 不貼錯', () => {
  // 存檔時 msgIndex 2 指的是 p1 的貼圖;取回時該位置換成 p3 的一般文字
  ...
  assert.equal(v.cells[0].skip, true, '目標訊息已變,必須跳過而非靜默覆蓋');
});

test('取回防呆:人物重排,用 personId 重新定位(不 skip、修正 personIndex)', () => {
  const messages = [];
  const people = [{ id: 'p2' }, { id: 'p1' }]; // 順序與存檔時對調
  const cells = [{ type: 'avatar', personIndex: 0, personId: 'p1' }];
  const v = validateFillCells(cells, messages, people);
  assert.equal(v.skipped, 0);
  assert.equal(v.cells[0].personIndex, 1, 'p1 現在在 index 1,應被重新定位到 1');
});

test('取回防呆:人物已被刪除 → 標 skip', () => {
  const v = validateFillCells([{ type: 'avatar', personIndex: 0, personId: 'gone' }], [], [{ id: 'p1' }]);
  assert.equal(v.skipped, 1);
