...
- **內建 AI 助手**：「AI」分頁下指令（例：生成一段媽媽提醒兒子帶傘的對話），AI 直接改畫面，可一鍵還原；自帶 API Key（Groq/OpenAI/Gemini/OpenRouter/Ollama/自訂）或選「刷作者的信用卡」免費體驗（每日限量，經站長的 [worker 代理](worker/README.md) 轉發，不儲存對話內容；fork 可自架）；編劇/評審與執行可分別設不同模型
